...
// Предварительно кодируем паттерны в байты
const encoder = new TextEncoder();
const PATTERNS = {
  snapshotBegin: encoder.encode("GameMapSnapShotBegin"),
  snapshotEnd: encoder.encode("GameMapSnapShotEnd"),
  recordBegin: encoder.encode("GameMapRecordBegin"),
} as const;

/**
 * Вспомогательная функция поиска подмассива в Uint8Array
 */
...
/**
 * Очищает бинарный поток, возвращая только читаемый ASCII и удаляя блоки карты
 */
function cleanRawBuffer(source: ArrayBuffer): Uint8Array {
  // Создаем Uint8Array view для ArrayBuffer
  const sourceView = new Uint8Array(source);

  // Этап 1: Фильтрация printable ASCII (0x20-0x7E) + \n + \r
  const filtered = new Uint8Array(sourceView.length);
  let writeIdx = 0;

  for (let i = 0; i < sourceView.length; i++) {
    const byte = sourceView[i]!;
    if (
      (byte >= CODES.SPACE && byte <= CODES.TILDE) ||
      byte === CODES.LF ||
      byte === CODES.CR
    ) {
      filtered[writeIdx++] = byte;
    } else {
      filtered[writeIdx++] = CODES.SPACE;
    }
  }

  // Срезаем лишнее, оставляя только записанное
  const data = filtered.subarray(0, writeIdx);

  // Этап 2: Удаляем блок снимка карты
  const cleaned = removeSnapshotBlock(data);

  // Этап 3: Обрезаем хвост после записи
  return truncateAfterRecord(cleaned);
}

function removeSnapshotBlock(data: Uint8Array): Uint8Array {
  const beginLen = PATTERNS.snapshotBegin.length;
  const endLen = PATTERNS.snapshotEnd.length;

  let result = data;
  let pos = 0;

  while (true) {
    const beginIdx = findPatternIndex(result, PATTERNS.snapshotBegin, pos);
    if (beginIdx === -1) break;

    const endIdx = findPatternIndex(
      result,
      PATTERNS.snapshotEnd,
      beginIdx + beginLen,
    );
    if (endIdx === -1) break;

    // Формируем новый массив: [до begin] + [маркеры] + [после end]
    const newLen =
      beginIdx + beginLen + 1 + endLen + (result.length - endIdx - endLen);
    const replacement = new Uint8Array(newLen);

    let offset = 0;
    // Часть до начала блока
    replacement.set(result.subarray(0, beginIdx), offset);
    offset += beginIdx;

    // Маркер начала
    replacement.set(PATTERNS.snapshotBegin, offset);
    offset += beginLen;

    // Разделитель \n
    replacement[offset++] = CODES.LF;

    // Маркер конца
    replacement.set(PATTERNS.snapshotEnd, offset);
    offset += endLen;

    // Остаток после блока
    replacement.set(result.subarray(endIdx + endLen), offset);

    result = replacement;
    // Смещаем позицию поиска, чтобы не зациклиться
    pos = beginIdx + beginLen + 1 + endLen;
  }

  return result;
}

function truncateAfterRecord(data: Uint8Array): Uint8Array {
  const recordIdx = findPatternIndex(data, PATTERNS.recordBegin);
  if (recordIdx === -1) return data;

  const cutoff = recordIdx + PATTERNS.recordBegin.length + 10000;
  if (cutoff >= data.length) return data;

  return data.subarray(0, cutoff);
}
...
self.onmessage = async (e: MessageEvent<File>) => {
  ...
  try {
    ...
    // Теперь типы совместимы: ArrayBuffer -> Uint8Array
    const cleanedBytes = cleanRawBuffer(buffer);

    // Если нужна строка для regex:
    const text = new TextDecoder().decode(cleanedBytes).replace(/\s+/g, " ");
    ...
    // --- 1) Sequential parsing of players section ---
    const playersSectionM = text.match(
      /\bplayers\b([\s\S]*?)(?=\bplayersinfo\b|\bPatternList\b|\bF\b|$)/,
    );
    ...
    const countExists = (arr: Player[]) =>
      arr.reduce((acc, p) => acc + (p.bexists ? 1 : 0), 0);
    const chosen =
      countExists(blkPlayers) > countExists(seqPlayers)
        ? blkPlayers
        : seqPlayers;
    gameInfo.players.push(...chosen);

    // --- 2) Extract playersinfo block and individual sic entries ---
    const playersInfoSectionM = text.match(
      /\bplayersinfo\b([\s\S]*?)(?=\bPatternList\b|\bF\b|$)/,
    );
    ...
    for (let i = 0; i < playersInfoEntries.length; i++) {
      ...
      if (matchedIdx === null) {
        while (
          fallbackIndex < gameInfo.players.length &&
          usedPlayerIdx.has(fallbackIndex)
        ) {
          fallbackIndex++;
        }
      ...
      if (
        matchedIdx !== null &&
        matchedIdx >= 0 &&
        matchedIdx < gameInfo.players.length
      ) {
